...
module.exports = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    //ChatGPT-4o와의 질문하고 답하기 처리 실시간 이벤트 수신기
    //프론트엔드 소켓에서 호출하는 gpt서버 이벤트 수신기 정의
    socket.on("gpt", async (msg) => {
      //Stpe0: 사용자가 보내준 메시지 데이터를 다시 현재사용자에게 발송해 화면에 표시하기(내가 질문한 내역을 미리 뿌려줌)
      // socket.emit("gptMessage",msg); 현재 메시지를
      ...
      //클-백 socket으로 실시간 // 백-ChatGPT 비동기방식으로 restful 통신
